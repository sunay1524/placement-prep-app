import { signUp, signInwithGit } from "@/lib/auth-actions";
...
export default async function Page({ searchParams }: PageProps) {
    ...
    return (
        <div className="min-h-[calc(100vh-4rem)] w-full flex flex-col items-center p-8 relative top-14">
            <h1 className="text-3xl font-extrabold text-slate-900 mb-8">Create an Account</h1>
            ...
            <div className="flex flex-col border border-slate-200 shadow-sm rounded-2xl p-8 gap-5 w-full max-w-md bg-white text-slate-900">
                ...
                <form action={signUp} className="flex flex-col gap-5">
                    <input name="name" type="text" placeholder="Name" required className="border border-zinc-300 bg-white text-slate-900 placeholder:text-slate-400 rounded-lg p-2.5 outline-none focus:ring-2 focus:ring-slate-900" />
                    ...
                    <button type="submit" className="bg-zinc-900 hover:bg-black text-white rounded-lg p-2.5 font-semibold transition-colors">Sign Up</button>
                </form>
